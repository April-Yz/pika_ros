...
/usr/bin/python3 ~/pika_ros/scripts/pika_calibrate_head_from_wrist_boards.py \
  --name head_d435_new_table \
  --left-handeye ~/pika_ros/calibration/handeye/left_wrist_new_table_eye_in_hand.json \
  --left-samples ~/pika_ros/calibration/handeye/samples_left_wrist_new_table.npz \
  --right-handeye ~/pika_ros/calibration/handeye/right_wrist_new_table_eye_in_hand.json \
  --right-samples ~/pika_ros/calibration/handeye/samples_right_wrist_new_table.npz \
  --left-base-T-right-base ~/pika_ros/calibration/handeye/left_base_T_right_base_new_table.json \
  --left-image-topic /gripper/camera_l/color/image_raw \
  --left-info-topic /gripper/camera_l/color/camera_info \
  --right-image-topic /gripper/camera_r/color/image_raw \
  --right-info-topic /gripper/camera_r/color/camera_info \
...
/usr/bin/python3 ~/pika_ros/scripts/estimate_dual_base_transform.py \
  ...
  --right-handeye ~/pika_ros/calibration/handeye/right_wrist_new_table_eye_in_hand.json \
  --right-samples ~/pika_ros/calibration/handeye/samples_right_wrist_new_table.npz \
  --output ~/pika_ros/calibration/handeye/left_base_T_right_base_new_table.json
```
...
## 2026-05-18 New Table 与 Try2 结果对比

本节记录 `new_table` 和原先 `try2` 的对比结论，以及两次 `head cam` 标定结果的差异。

### 左腕相机：`left_wrist_new_table` vs `left_wrist_try2`

`try2`：

- `sample_count = 16`
- `translation_mean_m = 0.00663`
- `rotation_mean_deg = 0.917`

`new_table`：

- `sample_count = 17`
- `translation_mean_m = 0.01291`
- `rotation_mean_deg = 0.563`
...
- `new_table` 左腕旋转残差更小，但平均平移残差比 `try2` 更大。
- 两个结果都可用，`try2` 在平移误差上更好，`new_table` 在旋转一致性上更好。

### 右腕相机：`right_wrist_new_table` vs `right_wrist_try2`

`try2`：

- `sample_count = 18`
- `translation_mean_m = 0.00938`
- `rotation_mean_deg = 1.094`

`new_table`：

- `sample_count = 15`
- `translation_mean_m = 0.01033`
- `rotation_mean_deg = 1.795`
...
- 右腕 `new_table` 和 `try2` 的位置差很小，只有约 `3 mm`。
- 但 `new_table` 的旋转残差更差，因此右腕 `try2` 仍然更稳。

### 左右 Base：`left_base_T_right_base_new_table` vs `left_base_T_right_base_try2`

`try2`：

- translation = `[-0.0236, -0.5718, 0.0205]`

`new_table`：

- translation = `[-0.0218, -0.8562, 0.0198]`
...
- `new_table` 下两只机械臂 base 间距比 `try2` 大了约 `28.4 cm`，这和“把两机械臂拉远”这一实际操作一致。
- 因为 `head cam` 的结果是相对于 `left_base` 表达的，只要 `left_base` 和 `right_base` 的相对关系变化，`head cam` 在 `left_base` 下的数值就会跟着变化。

### Head Cam：`head_d435_new_table` / `head_d435_new_table_0515` vs `head_d435_try2`

`try2`：

- `sample_count = 27`
- `translation_mean_m = 0.00877`
- `rotation_mean_deg = 0.816`

`new_table`：

- `sample_count = 21`
- `translation_mean_m = 0.00591`
- `rotation_mean_deg = 0.574`

`new_table_0515`：

- `sample_count = 16`
- `translation_mean_m = 0.00480`
- `rotation_mean_deg = 0.607`

与 `try2` 的差异：

- `head_d435_try2 -> head_d435_new_table`
  - 平移变化约 `0.162 m`
  - 旋转变化约 `7.35 deg`
- `head_d435_try2 -> head_d435_new_table_0515`
  - 平移变化约 `0.123 m`
  - 旋转变化约 `5.60 deg`
- `head_d435_new_table -> head_d435_new_table_0515`
  - 平移变化约 `0.0393 m`
  - 旋转变化约 `4.00 deg`

判断：

- `new_table` 两次 `head cam` 标定的残差都很好，甚至比 `try2` 更小。
- 但相对于 `try2` 的 `left_base_T_head_camera` 变化明显，尤其是姿态变化达到 `5.6-7.35 deg`。

### 为什么 `head cam` 朝向变化会比较大

你提到自己“没有动 head cam，只是把两个机械臂的 base 拉远了”。从结果看，这种情况完全可能让 `left_base_T_head_camera` 的数值变化明显，原因有三层：

1. `head cam` 外参是相对于 `left_base` 表达的，不是相对于桌子/world 表达的。
   - 只要 `left_base` 相对于桌子的位置或朝向有变化，`left_base_T_head_camera` 就会变。

2. `new_table` 的 `left_base_T_right_base` 相对 `try2` 不只是平移变了约 `28.4 cm`，旋转也变了约 `1.89 deg`。
   - 这说明重摆机械臂后，base 参考系本身就有轻微姿态变化。
...
3. `head cam` 标定是间接标定：
   ...
   - FK 误差
   - Charuco 检测误差
   会一起叠加到 `left_base_T_head_camera` 上。

所以：

- `head cam` 朝向变化不一定代表你真的动了 `head cam`
- 更可能是：
  - base 重新摆放后参考系变了
  - 加上间接标定链路的误差叠加
...
- 如果你要在当前 `new_table` 这套桌面布局下继续用系统，应优先使用 `new_table` 对应的整套参数，不要混用 `try2` 的 base 或 head 外参。
- 如果你需要和旧桌面布局比较，应把对比放在统一世界参考下，而不是只看 `left_base_T_head_camera` 数值本身。
- 如果要判断 `head cam` 是否真的被机械移动过，最好的方法不是直接看 `left_base_T_head_camera`，而是：
  - 固定桌面上的同一块 Charuco 板
  - 用同一套 base 参考重新观测
  - 或者使用一个桌面/world 固定参考系去比较

# head cam
 s5 的问题在于它会带上 data_tools 那一路采集/转发，不适合纯标定。

  推荐的轻量启动方式

  方式 1：直接只起 D435 color，最适合你现在这个 head cam 标定
  ...
  这个最轻。因为你当前 head_from_wrist 标定脚本只用 RGB 和 camera_info，不需要 depth。

  方式 2：如果你后面还要录 RGBD，再起 color+depth
  ...
  但这比纯 color 更重，不建议用于纯标定。

  方式 3：如果你要脚踏板录 head D435 数据
  仓库里已有轻量入口：

  - pika_ros/scripts/start_head_d435_rgbd_pedal.sh

  它会只起 D435 + 录制器，不走 s5 那套多路采集。

  ———

  你现在这步 head cam 标定的建议组合

  1. 起双臂链路
  2. 起左右腕相机
  3. 单独起 D435 color-only
  4. 跑：
  ...
  /usr/bin/python3 ~/pika_ros/scripts/pika_calibrate_head_from_wrist_boards.py \
    ...
    --right-handeye ~/pika_ros/calibration/handeye/right_wrist_new_table_eye_in_hand.json \
    --right-samples ~/pika_ros/calibration/handeye/samples_right_wrist_new_table.npz \
    --left-base-T-right-base ~/pika_ros/calibration/handeye/
left_base_T_right_base_new_table.json \
  ...
  为什么这比 s5 好
  因为 s5 对应的 launch 里除了 rs_camera.launch，还会 include：

  - run_data_capture_multi_pika_teleop_d435...
  - 缓冲/转发/采集服务
  ...
  如果你愿意，我可以下一步直接把 pika_ros/scripts/md/CHARUCO_HANDEYE_CALIBRATION.md 里 head
  cam 这一段改成“默认推荐只起 D435 color-only，不要走 s5”。
