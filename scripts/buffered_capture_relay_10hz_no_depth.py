...
class BufferedCaptureRelay:
    def __init__(self, dataset_dir: str, publish_hz: float):
        ...
        self.log_path = self.dataset_dir / "buffered_capture_relay_10hz_no_depth.log"
    ...
    def run(self):
        rospy.init_node("buffered_capture_relay_10hz_no_depth", anonymous=True)
        self.setup_relays()
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Republish non-fisheye and non-depth capture topics at a fixed rate using the latest received message."
    )
